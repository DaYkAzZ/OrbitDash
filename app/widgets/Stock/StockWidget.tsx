...
import type { WidgetProps, StockWidgetData } from '../../types';
...
export function StockWidget({ widget, mode, onFocus, onFullscreen, onClose }: WidgetProps) {
  const data = widget.data as StockWidgetData;
    ...
    return (
      ...
      >
        <div className="flex items-start justify-between">
          ...
          <span className={`rounded-lg px-2 py-0.5 text-xs font-semibold ${bg} ${color}`}>
    ...
    return (
      <div className="flex flex-col gap-4 p-6">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-lg font-semibold text-foreground">{data.label}</h2>
        ...
        <div className="grid grid-cols-2 gap-3">
          {[
            ...
            { label: 'Variation', value: quote ? formatPct(quote.dp) : '—', colored: true },
            { label: 'Ouverture', value: quote ? formatPrice(quote.o) : '—' },
            { label: 'Clôture préc.', value: quote ? formatPrice(quote.pc) : '—' },
            { label: 'Haut', value: quote ? formatPrice(quote.h) : '—' },
            { label: 'Bas', value: quote ? formatPrice(quote.l) : '—' },
          ].map(({ label, value, colored }) => (
            <div key={label} className="rounded-xl bg-zinc-100 dark:bg-zinc-800/60 p-3">
              <p className="text-xs text-zinc-500">{label}</p>
              <p className={`text-sm font-semibold ${colored ? color : 'text-foreground'}`}>
                {value}
              </p>
            </div>
          ))}
        ...
        {/* Top actus */}
        <div>
          <p className="mb-2 text-xs font-semibold uppercase tracking-wider text-zinc-500">Actualités</p>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="text-right">
          ...
          <p className={`text-lg font-semibold ${color}`}>
            {quote ? formatPct(quote.dp) : ''} ({quote ? formatPrice(Math.abs(quote.d)) : ''})
          </p>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="flex flex-col gap-2 max-h-64 overflow-y-auto pr-1">
          {news.map((n) => (
            ...
            >
              <p className="text-sm font-medium text-white group-hover:text-indigo-400 line-clamp-2">{n.headline}</p>
              <p className="mt-1 text-xs text-zinc-500">{n.source} · {timeAgo(n.datetime)}</p>
            </a>
          ))}
        </div>
      </div>
