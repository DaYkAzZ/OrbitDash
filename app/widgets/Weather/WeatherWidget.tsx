...
import type { WidgetProps, WeatherWidgetData } from '../../types';
...
export function WeatherWidget({ widget, mode, onFocus, onFullscreen, onClose }: WidgetProps) {
  const data = widget.data as WeatherWidgetData;
  ...
  const hourly = weather.hourly.time
    .map((t, i) => ({ time: t, temp: weather.hourly.temperature_2m[i], code: weather.hourly.weathercode[i] }))
    .filter((_, i) => {
      ...
      <div className="flex flex-col gap-4 p-6">
        ...
        <div className="flex items-center gap-4 rounded-2xl bg-zinc-100 dark:bg-zinc-800/60 p-4">
          ...
          <div className="flex flex-col">
            <p className="text-3xl font-bold text-foreground">{temp(cur.temperature_2m, data.unit)}</p>
            <p className="text-sm text-zinc-500">{label}</p>
            <p className="text-xs text-zinc-500">
              💧{cur.relativehumidity_2m}% · 💨{Math.round(cur.windspeed_10m)} km/h
            </p>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="flex gap-2 overflow-x-auto pb-1">
            {hourly.map((h) => (
              <div key={h.time} className="flex min-w-[56px] flex-col items-center gap-1 rounded-xl bg-zinc-100 dark:bg-zinc-800/60 p-2">
                <p className="text-xs text-zinc-500">
                  {new Date(h.time).getHours()}h
                </p>
                <span className="text-xl">{wmo(h.code).icon}</span>
                <p className="text-xs font-semibold text-foreground">{temp(h.temp, data.unit)}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="flex flex-col gap-1">
            {weather.daily.time.map((day, i) => (
              <div key={day} className="flex items-center justify-between rounded-xl px-3 py-2 hover:bg-zinc-100 dark:hover:bg-zinc-800/40">
                ...
                <span className="text-lg">{wmo(weather.daily.weathercode[i]).icon}</span>
    ...
    <div className="flex w-full max-w-2xl flex-col gap-6 p-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <p className="text-6xl font-bold text-white">{temp(cur.temperature_2m, data.unit)}</p>
        </div>
      </div>

      <div className="grid grid-cols-3 gap-3">
        {[
          { label: 'Ressenti', value: temp(cur.apparent_temperature, data.unit) },
      ...
      <div>
        <p className="mb-3 text-sm font-semibold uppercase tracking-wider text-zinc-400">7 jours</p>
        <div className="grid grid-cols-7 gap-2">
          {weather.daily.time.map((day, i) => (
            <div key={day} className="flex flex-col items-center gap-1 rounded-xl bg-zinc-800/60 p-2">
              <p className="text-xs text-zinc-500 capitalize">
                {new Date(day).toLocaleDateString('fr-FR', { weekday: 'short' })}
              </p>
              <span className="text-2xl">{wmo(weather.daily.weathercode[i]).icon}</span>
